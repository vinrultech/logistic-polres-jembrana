  ...
  <v-card class="elevation-6">
    <v-card-title>
      ...
      <v-avatar color="primary" size="48">
        <img :src="`${host}upload/${item.foto_petugas_ukur}`" />
      </v-avatar>
    </v-card-title>
    <v-card-text>
      <v-row>
      ...
      <v-stepper alt-labels>
        <v-stepper-header>
          ...
          <v-stepper-step step="2">
            <div class="text-center text-step">
              Petugas Ukur<br />
              <span v-if="item.nama_petugas_ukur">{{
              ...
              <div
                v-if="
                  item.bidangs.filter((el) => el.status_ukur === 1).length > 0
                "
              >
                <v-icon color="success" size="20">fas fa-user-check</v-icon
                ><span class="done">
                  {{ item.bidangs.filter((el) => el.status_ukur === 1).length }}
                  bidang</span
                >
              </div>
              <div
                v-if="
                  item.bidangs.filter((el) => el.status_ukur === 0).length > 0
                "
              >
                <v-icon color="error" size="20">fas fa-user-times</v-icon
                ><span class="undone">
                  {{ item.bidangs.filter((el) => el.status_ukur === 0).length }}
                  bidang</span
                >
              </div>
            </div>
          ...
          <v-stepper-step step="3">
            <div class="text-center text-step">Petugas Gambar</div>
          </v-stepper-step>
          <v-divider></v-divider>
