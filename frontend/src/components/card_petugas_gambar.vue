      ...
      <v-stepper alt-labels>
        <v-stepper-header>
          ...
          <v-stepper-step complete step="2" color="success">
            <div class="text-center text-step">Petugas Ukur</div>
          </v-stepper-step>
          <v-divider></v-divider>
          <v-stepper-step step="3">
            <div class="text-center text-step">
              Petugas Gambar<br />
              <span v-if="item.nama_petugas_gambar">{{ item.nama_petugas_gambar }}</span><span v-else>{{ item.petugas }}</span><br />
              <div
                v-if="
              ...
              <div
                v-if="
                  item.bidangs.filter((el) => el.status_gambar === 0 && el.status_problem === 0).length > 0
                "
              >
                <v-icon color="error" size="20">fas fa-user-times</v-icon
                ><span class="undone">
                  {{ item.bidangs.filter((el) => el.status_gambar === 0 && el.status_problem === 0).length }}
                  bidang</span
                >
              </div>
              <div
                v-if="
                  item.bidangs.filter((el) => el.status_problem === 1 && el.status_gambar === 0).length > 0
                "
              >
                <v-icon color="error" size="20">fas fa-exclamation-triangle</v-icon
                ><span class="undone">
                  {{ item.bidangs.filter((el) => el.status_problem === 1 && el.status_gambar === 0).length }}
                  bidang</span
                >
              </div>
            </div>
