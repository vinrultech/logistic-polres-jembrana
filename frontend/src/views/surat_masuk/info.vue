...
import { SURAT_MASUK, INFO } from "../../breadcum";
...
export default {
  ...
  data: () => ({
    breadcums: utils.breadcumTwo(SURAT_MASUK(false), INFO),
    row_id: null,
    is_loaded: false,
  }),
  computed: {
    ...mapGetters({
      item: "surat_masuk/get_item",
    }),
  },
  methods: {
    back() {
  ...
  mounted() {
    ...
    this.$store.dispatch("surat_masuk/fetch", row_id).then(() => {
      this.is_loaded = true;
    });
  },
};
