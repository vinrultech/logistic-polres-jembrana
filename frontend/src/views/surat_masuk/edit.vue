...
import { SURAT_MASUK, EDIT } from "../../breadcum";
...
export default {
  ...
  data: () => ({
    breadcums: utils.breadcumTwo(SURAT_MASUK(false), EDIT),
    id: null,
    no_surat: null,
    tanggal_surat: null,
    dari: null,
  ...
  methods: {
    update() {
      if (this.$refs.form.validate()) {
        this.$store.dispatch("surat_masuk/update", {
          item: {
            kode: this.kode,
            nama: this.nama,
          },
  ...
  mounted() {
    ...
    const item = this.$store.getters["surat_masuk/item"](parseInt(id));
