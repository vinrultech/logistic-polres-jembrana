...
import { UNIT_KERJA, EDIT } from "../../breadcum";
...
export default {
  ...
  data: () => ({
    breadcums: utils.breadcumTwo(UNIT_KERJA(false), EDIT),
    id: null,
    alamat: null,
    nama: null,
    telepon: null,
  ...
  methods: {
    update() {
      if (this.$refs.form.validate()) {
        this.$store.dispatch("unit_kerja/update", {
          item: {
            alamat: this.alamat,
            nama: this.nama,
  ...
  mounted() {
    ...
    const item = this.$store.getters["unit_kerja/item"](parseInt(id));
