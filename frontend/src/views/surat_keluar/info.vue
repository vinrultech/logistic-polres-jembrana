      ...
      <v-col cols="12">
        <v-card class="elevation-6" v-if="is_loaded">
          <v-card-text>
            ...
            <v-row>
              <v-col cols="4"> No Surat </v-col>
              <v-col cols="8">
                {{ item.no_surat }}
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="4"> Tanggal Surat </v-col>
              <v-col cols="8">
                {{ item.tanggal_surat }}
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="4"> Tujuan </v-col>
              <v-col cols="8">
                {{ item.tujuan }}
              </v-col>
            </v-row>
...
import { SURAT_KELUAR, INFO } from "../../breadcum";
...
export default {
  ...
  data: () => ({
    breadcums: utils.breadcumTwo(SURAT_KELUAR(false), INFO),
    row_id: null,
    is_loaded: false,
  }),
  computed: {
    ...mapGetters({
      item: "surat_keluar/get_item",
    }),
  },
  methods: {
    back() {
  ...
  mounted() {
    ...
    this.$store.dispatch("surat_keluar/fetch", row_id).then(() => {
      this.is_loaded = true;
    });
  },
};
