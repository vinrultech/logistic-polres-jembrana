    ...
    <v-row>
      <template v-for="item in items">
        <v-col cols="12" xl="6" lg="6" md="6" sm="12" xs="12" :key="item.id">
          <card-petugas-gambar :item="item" :is_detail="true" @bidang="bidang" @detail="detail" />
        </v-col>
      </template>
    </v-row>
    <paging
...
import BidangPetugasGambar from "../../components/bidang_petugas_gambar";
import CardPetugasGambar from "../../components/card_petugas_gambar";
import Breadcum from "../../components/breadcum";
import { PETUGAS_GAMBAR } from "../../breadcum";
...
export default {
  ...
  data: () => ({
    breadcums: utils.breadcumOne(PETUGAS_GAMBAR(true)),
    filters: [
      {
        id: "no_berkas",
        nama: "No Berkas",
  ...
  methods: {
    ...
    detail(item) {
      this.$router.push(`/admin/petugas_gambar/detail/${item.id}`);
    },
    get(action) {
      let no = 0;
      if (action === "next") {
      ...
      this.$store.dispatch("petugas_gambar/paginator", query);
    },
    previous() {
      this.get("prev");
    },
    ...
    getFirst() {
      this.$store.dispatch("petugas_gambar/null_items").then(() => {
        this.get("first");
      });
    }
  },
