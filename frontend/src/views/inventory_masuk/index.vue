...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      last_id: "inventory_masuk/last_id",
    }),
  },
  watch: {
    limit: function () {
  ...
  methods: {
    ...
    async remove(id) {
      ...
      this.$swal({
        ...
        if (result.value) {
          this.$store.dispatch("inventory_masuk/remove", id).then(() => {
            this.refresh();
          });
        }
      });
    ...
    get() {
      this.$store.dispatch("inventory_masuk/gets", {
        last_id: this.last_id,
        limit: this.limit.value,
      });
    },
    previous() {
      this.$store.dispatch("inventory_masuk/prev");
    },
    next() {
      this.$store.dispatch("inventory_masuk/next", this.isSearch);
    },
    cari() {
      this.$store.dispatch("inventory_masuk/reset");
      this.$store.dispatch("inventory_masuk/search", {
        last_id: this.last_id,
        limit: this.limit.value,
  ...
  mounted() {
    this.refresh();
    //this.$store.dispatch("inventory_masuk/gets", { last_id: this.last_id, limit : 10 });
  },
};
</script>
