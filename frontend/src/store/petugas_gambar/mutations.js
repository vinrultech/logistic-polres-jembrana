...
export default {
    ...
    PAGINATOR_PEOPLE: (state, payload) => {
        state.peoples = _.concat(state.peoples, payload.items)
        state.last_no = payload.last_no
        state.first_no = payload.first_no
        //state.prev_show = payload.prev_show
        //state.next_show = payload.next_show
    ...
    CHANGE_DATE: (state, payload) => {
        ...
        state.items[index].bidangs[idx].tanggal_gambar = payload.tanggal_gambar

    },
    CHANGE_STATUS_MASALAH: (state, payload) => {
