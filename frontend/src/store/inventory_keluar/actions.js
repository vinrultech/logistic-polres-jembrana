...
export default {
    gets: injector.encase(['axios', 'toastr'], (axios, toastr) => (context, payload) => {
        ...
        axios.get(`/admin/inventory_keluar?${q}`)
            .then((response) => {
    ...
    search: injector.encase(['axios', 'toastr'], (axios, toastr) => (context, payload) => {
        ...
        axios.get(`/admin/inventory_keluar/search?${q}`)
            .then((response) => {
                const items = response.data;
    ...
    create: injector.encase(['axios', 'router'], (axios, router) => (context, item) => {
        ...
        axios.post(`/admin/inventory_keluar/create`, item, {
                message: "inventory_keluar berhasil di tambah"
            })
            .then(() => {
                router.push("/admin/inventory_keluar")
            }).catch((error) => {
    ...
    remove: injector.encase(['axios'], (axios) => (_, row_id) => {
        return new Promise((resolve) => {
            axios.delete(`/admin/inventory_keluar/remove/${row_id}`, {
                    message: "inventory_keluar berhasil di hapus"
                })
                .then(() => {
                    resolve(true);
                })
