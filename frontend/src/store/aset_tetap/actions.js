...
export default {
    fetch: injector.encase(['axios'], (axios) => (context, payload) => {
        ...
        axios.get(`/admin/aset_tetap/${row_id}`)
            .then((response) => {
    ...
    all: injector.encase(['axios'], (axios) => (context) => {
        return new Promise((resolve) => {
            axios.get(`/admin/aset_tetap/all`)
            .then((response) => {
                const items = response.data;
    ...
    search: injector.encase(['axios', 'toastr'], (axios, toastr) => (context, payload) => {
        ...
        axios.get(`/admin/aset_tetap/search?${q}`)
            .then((response) => {
                const items = response.data;
    ...
    create: injector.encase(['axios', 'router'], (axios, router) => (context, item) => {
        ...
        axios.post(`/admin/aset_tetap/create`, item, {
                message: "Aset Tetap berhasil di tambah"
            })
            .then(() => {
                router.push("/admin/aset_tetap")
            }).catch((error) => {
    ...
    remove: injector.encase(['axios'], (axios) => (_, row_id) => {
        return new Promise((resolve) => {
            axios.delete(`/admin/aset_tetap/remove/${row_id}`, {
                    message: "Aset Tetap berhasil di hapus"
                })
                .then(() => {
                    resolve(true);
                })
    ...
    update: injector.encase(['axios', 'router'], (axios, router) => (context, payload) => {
        ...
        axios.put(`/admin/aset_tetap/update/${row_id}`, item, {
                message: "Aset Tetap berhasil di update"
            })
            .then(() => {
                router.push("/admin/aset_tetap")
            }).catch((error) => {
